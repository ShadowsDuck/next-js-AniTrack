...
export default function AnimeRelationSection({
  character,
}: {
  ...
  if (character?.anime.length === 0)
    return (
      <div className="flex items-center justify-center py-32">
        <p className="text-muted-foreground text-sm">No anime available.</p>
      </div>
    );

  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
      {character?.anime?.map((anime: AnimeRelation) => (
        <Link
            ...
            <div className="flex gap-4">
              ...
              <div className="flex min-w-0 flex-1 flex-col gap-2">
                ...
                <p className="text-muted-foreground text-sm">
                  Role: <span className="font-medium">{anime.role}</span>
                </p>
              </div>
            </div>
          </div>
