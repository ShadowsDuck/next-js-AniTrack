...
export default function MangaRelationSection({ people }: { people: People }) {
  if (people?.manga?.length === 0)
    return (
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
      {people?.manga?.map((manga: People["manga"][number]) => (
        <Link
            ...
            <div className="flex gap-4">
              ...
              <div className="flex min-w-0 flex-1 flex-col gap-2">
                ...
                <p className="text-muted-foreground text-sm">
                  Position:{" "}
                  <span className="font-medium">{manga.position}</span>
                </p>
              </div>
            </div>
          </div>
